...
describe Cat do
  ...
  subject(:aurora) { described_class.new('Aurora', 'Mixed', 'Grey') }

  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'Cat has a shared method name with other classes' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(aurora).to_not be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(aurora).to be_hiding
  end
end
