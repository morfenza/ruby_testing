...
describe NumberGame do
  ...
  describe '#game_over?' do
    ...
    context 'when user guess is not correct' do
      ...
      subject(:game_not_end) { described_class.new(7, '2')}
  ...
  describe '#player_turn' do
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        incorrect_input = 'bangui'
        correct_input = '2'
        allow(game_loop).to receive(:player_input).and_return(incorrect_input, incorrect_input, correct_input)
      end
  ...
  describe '#final_message' do
    ...
    context 'when count is 4 and over' do
      ...
      subject(:game_three) { described_class.new(9, '9', 4) }

      # remove the 'x' before running this test
      it 'outputs correct phrase' do
        difficult_phrase = "That was hard. It took you 4 guesses!\n"
        expect { game_three.final_message }.to output(difficult_phrase).to_stdout
      end
    end
  end
end
