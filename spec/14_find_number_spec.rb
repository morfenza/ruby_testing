...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      ...
      subject(:game_victory) { described_class.new(0, 9, random_number, 8) }

      it 'is game over' do
        expect(game_victory).to be_game_over
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      subject(:game_failure) { described_class.new(0, 9, random_number, 4) }

      it 'is not game over' do
        expect(game_failure).to_not be_game_over
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        expect { edge_guess_game.update_range }.to change { edge_guess_game.min }.to(edge_guess_game.max)
      end

      it 'does not update max' do
        expect { edge_guess_game.update_range }.to_not(change { edge_guess_game.max })
      end
    end
  end
end
