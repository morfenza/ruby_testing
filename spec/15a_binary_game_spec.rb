...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        valid_input = '10'
        invalid_input_sym = ':a'
        allow(game_input).to receive(:gets).and_return(invalid_input_sym, valid_input)
      end
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        valid_input = '10'
        invalid_input_sym = ':a'
        invalid_input_str = 'mermas'
        allow(game_input).to receive(:gets).and_return(invalid_input_sym, invalid_input_str, valid_input)
      end
  ...
  describe '#verify_input' do
    ...
    context 'when given a valid input as argument' do
      subject(:valid_input_game) { described_class.new(1, 10) }

      it 'returns valid input' do
        min = valid_input_game.instance_variable_get(:@minimum)
        max = valid_input_game.instance_variable_get(:@maximum)
        valid_input = 5
        expect(valid_input_game.verify_input(min, max, valid_input)).to eq(valid_input)
      end
    end

    context 'when given invalid input as argument' do
      subject(:invalid_input_game) { described_class.new(1, 10) }

      it 'returns nil' do
        min = invalid_input_game.instance_variable_get(:@minimum)
        max = invalid_input_game.instance_variable_get(:@maximum)
        invalid_input = 15
        expect(invalid_input_game.verify_input(min, max, invalid_input)).to be_nil
      end
    end
  end
  ...
  describe '#display_turn_order' do
    ...
    subject(:game_guess) { described_class.new(1, 10) }
    ...
    before do
      ...
      allow(game_guess).to receive(:display_guess)
    end

    # Command Method -> Test the change in the observable state
    it 'increases guess_count by one' do
      expect { game_guess.display_turn_order(binary_search) }.to change { game_guess.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(binary_search).to receive(:make_guess).once
      game_guess.display_turn_order(binary_search)
    end
